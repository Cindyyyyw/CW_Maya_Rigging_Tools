...
def build_aim_constraint():
    jnt_lis= cmds.ls(sl=1)
    aim_obj = 'l_eye_back_loc'
    for jnt in jnt_lis:
        cmds.aimConstraint(aim_obj,jnt, aim=(-1,0,0) ,wuo='l_eye_up_loc', wut= 'object')
        


def build_joint_on_crv():
    crv = cmds.ls(sl=1)[0]
    for i in range(30):
        jnt_name = "r_eyelid_mid_"+str(i+1)+"_jnt"
